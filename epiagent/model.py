...
class EpiAgent(nn.Module):
    ...
    def forward(self, 
        ...
        Args:
            ...
            return_SD_output (bool): If True, includes outputs from the signal decoder in the return dictionary (default is False).
...
class EpiAgent_BC(EpiAgent):
    """
    A variant of the EpiAgent model designed specifically for batch correction tasks.
    This model introduces a batch-specific token embedding to correct batch effects 
    and generates batch-invariant cell embeddings.
    
    Args:
        batch_num (int): The number of batches (or datasets) from which the data is derived. 
                          This is necessary for batch correction and must be specified.
        vocab_size (int): The size of the vocabulary (default is 1355449).
        num_layers (int): Number of layers in the transformer encoder (default is 18).
        embedding_dim (int): The dimensionality of the embeddings (default is 512).
        num_attention_heads (int): The number of attention heads in each transformer layer (default is 8).
    ...
    def __init__(self, 
                 ...
                 pos_weight_signals=torch.tensor(100),
                 use_flash_attn=True):
        super(EpiAgent_BC, self).__init__(vocab_size, num_layers, embedding_dim, num_attention_heads, 
                                          max_rank_embeddings, MLP_hidden_for_RLM, MLP_hidden_for_CCA,
                                          pos_weight_for_RLM, pos_weight_for_CCA, pos_weight_signals, use_flash_attn)
        
        # Batch correction specific components: embedding for batch-specific information
        self.batch_embedding = nn.Embedding(batch_num, embedding_dim)
    ...
    def forward(self, 
                ...
                signals=None):
        """
        Forward pass for the EpiAgent_BC model with batch correction. If batch_ids are provided, 
        batch-specific embeddings are added to the cCRE embeddings to correct for batch effects.
        
        Args:
            input_ids (tensor): Input tensor of shape [batch_size, seq_len].
            batch_ids (tensor, optional): The batch indices for each cell in the batch (default is None).
            return_transformer_output (bool): If True, includes transformer outputs in the return dictionary (default is True).
            return_SD_output (bool): If True, includes outputs from the signal decoder in the return dictionary (default is False).
            calculate_rlm_loss (bool): If True, computes the RLM loss (default is False).
            calculate_cca_loss (bool): If True, computes the CCA loss (default is False).
        ...
        # Compute embeddings for cCREs
        ccre_embeddings = self.cCRE_embedding(input_ids)
        
        # Add batch-specific embeddings if batch_ids is provided
        if batch_ids is not None:
            batch_embeddings = self.batch_embedding(batch_ids)
            ccre_embeddings[:, 0, :] = ccre_embeddings[:, 0, :] + batch_embeddings
...
class EpiAgent_PT(nn.Module):
    ...
    def __init__(self, 
                 ...
                 num_layers_GNN=1,
                 all_pert_list=None):
        super(EpiAgent_PT, self).__init__()
        ...
        self.all_pert_list = all_pert_list
        self.num_perts = len(all_pert_list)
        self.num_layers = num_layers_GNN
        self.GO = torch.tensor(GO)
        self.GO_weight = torch.tensor(GO_weight)
        self.sim_layers = torch.nn.ModuleList()
        for i in range(1, self.num_layers + 1):
            self.sim_layers.append(SGConv(embedding_dim, embedding_dim, 1))
        self.layer_norm_for_GCN = nn.LayerNorm(embedding_dim)
    ...
    def forward(self, 
        ...
        # GNN
        pert_global_idx = torch.LongTensor(list(range(self.vocab_size)[-self.num_perts:])).to(input_ids.device)
        pert_global_emb = self.cCRE_embedding(pert_global_idx)

        # augment global perturbation embedding with GNN
        for idx, layer in enumerate(self.sim_layers):
            pert_global_emb = layer(pert_global_emb, self.GO.to(input_ids.device), self.GO_weight.to(input_ids.device))
            if idx < self.num_layers - 1:
                pert_global_emb = pert_global_emb.relu()

        pert_global_emb = self.layer_norm_for_GCN(pert_global_emb)

        # Compute embeddings
        ccre_embeddings = self.cCRE_embedding(input_ids)
        for idx, item in enumerate(input_ids):
            if item[0] != 1:
                new_emb = pert_global_emb[item[0]-self.vocab_size+len(self.all_pert_list),:]
                ccre_embeddings[idx,0,:] = new_emb
